...
import { create as createVector } from '../../../src/reversi/vector/Vector'
...
describe('Board', () => {
  ...
  it('getCellTypeDistribution should return board cell types distribution', () => {
    ...
    const board = create(4, 4)
    expect(getCellTypeDistribution(board)).toEqual({[TYPE_WHITE]: 2, [TYPE_BLACK]: 2, [TYPE_EMPTY]: 12});
  ...
  it('validateSize should throw an Error on invalid board size', () => {
    ...
    expect(oddBoardSizeCreation).toThrowError();
    expect(smallBoardSizeCreation).toThrowError();
  ...
  it('getFlippedCellFromCellChangeInDirection should return flipped Cell from cell change in direction', () => {
    ...
    expect(getFlippedCellFromCellChangeInDirection(createCell(2, 4, TYPE_WHITE), board, createVector(1, 0))).toEqual([])
    expect(getFlippedCellFromCellChangeInDirection(createCell(2, 3, TYPE_WHITE), board, createVector(1, 0))).toEqual([createCell(3, 3, TYPE_WHITE)])
    expect(getFlippedCellFromCellChangeInDirection(createCell(2, 4, TYPE_WHITE), board, createVector(-1, -1))).toEqual([])
  ...
  it('getFlippedCellFromCellChangeInDirection should return flipped Cell from cell change in direction', () => {
    ...
    const board = create(8, 8)
    expect(getFlippedCellsFromCellChange(createCell(2, 3, TYPE_WHITE), board)).toEqual([createCell(3, 3, TYPE_WHITE)])
